...
import org.openurp.std.graduation.model.{DegreeResult, GraduateResult}
import org.openurp.std.info.model.Examinee

class GraduateResultExtractor(entityDao: EntityDao) extends DefaultPropertyExtractor {

  override def get(bean: Object, name: String): Any = {
    if (name == "examinee.code") {
      val std = bean.asInstanceOf[GraduateResult].std
