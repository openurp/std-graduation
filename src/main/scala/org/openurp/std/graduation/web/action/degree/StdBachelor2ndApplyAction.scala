...
class StdBachelor2ndApplyAction extends ActionSupport, EntityAction[Bachelor2ndApply], ProjectSupport {
  ...
  def index(): View = {
    ...
    grs match {
      case Some(gr) =>
        val daQuery = OqlBuilder.from(classOf[Bachelor2ndApply], "da")
  ...
  def remove(): View = {
    ...
    val da = entityDao.get(classOf[Bachelor2ndApply], id)
    val std = getStudent
    if (da.std == std) {
      entityDao.remove(da)
