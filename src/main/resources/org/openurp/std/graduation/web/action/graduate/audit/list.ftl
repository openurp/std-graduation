...
[/#function]
  ...
  [@b.grid items=results var="result"]
    [@b.gridbar]
        ...
        var menu3 = bar.addMenu("自动审核", "autoAudit('result')");
        ...
        var exportMenu = bar.addMenu("导出",action.exportData("std.code:学号,std.name:姓名,std.gender.name:性别,std.person.code:身份证号,"+
               ...
               "std.enrollOn:入学日期,std.graduateOn:预计毕业日期,std.state.status.name:学籍状态,graduate.passed:是否通过," +
               "graduate.educationResult.name:毕结业结论,"+
               "std.state.squad.master.name:辅导员(班主任)",
                null,'fileName=毕业审核名单'));
    ...
    [@b.row]
      [@b.boxcol/]
        ...
        [@b.col title="专业/方向" property="std.state.major.name" width="19%"]
          ${(result.std.state.major.name)!} ${(result.std.state.direction.name)!}
        [/@]
          [@b.col title="是否通过&nbsp;<a href='!info?id='+result.id onclick='javascript:toogleResult()' title='查看详情'>详情</a>" width="15%"]
            [#if (result.passed)??]
              [@b.a href="!info?id=${result.id}" style="color:${result.passed?string('green', 'red')}" title=result.comments!""]${result.passed?string("通过", "未通过")}[/@]
                <div name="result.extra" style="display:none;text-align:center;">
                [#list result.items as resultItem]
                  ${resultItem.name}（${passedStr(resultItem)}）
                    [#if resultItem.comments?exists]： ${(resultItem.comments?replace("\r\n", "<br>")?replace("\n", "<br>"))?if_exists}[/#if]
                    [#if resultItem_has_next]<br>[/#if]
                [/#list]
                </div>
            [#else]
              未审核
        ...
        <input type="hidden" id="passed_${(result.id)!}" value="${(result.passed?string('1', '0'))!}" />
          [/@]
          [@b.col title="结论" property="educationResult" width="7%"]
            <span id="graduateState_${(result.id)!}">${(result.educationResult.name)!}</span>
          [/@]
          [@b.col title="状态"  width="8%"]
             [#if result.locked]
